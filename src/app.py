...
# ---------------------------
DEFAULT_IMATRIX_FILE = os.path.join("src", "gguf", "imatrix.dat")
DEFAULT_CALIBRATION_FILE = os.path.join("src", "gguf", "calibration_datav3.txt")
...
def run_command(command: str):
    ...
    while True:
        if stop_event.is_set():
            process.kill()
            yield "[INFO] Process cancelled by user.\n"
            break
...
def generate_custom_model_card(model_id, base_model_name, quant_method, username, save_folder, license="mit", datasets=None):
    ...
    custom_metadata = {
        ...
        "language": ["en"],
        "license": license,
        "tags": ["SpongeQuant", "quantized", formatted_qtype],
    }
...
def quantize_gptq(model_id: str, additional_param: str, hf_token: str, username: str):
    try:
        ...
        yield "[INFO] Loading model with integrated GPTQ configuration from local model directory...\n"
        ...
        yield f"[ERROR] Error during GPTQ quantization: {e}\n"
...
def quant_tavern_ui(model_ids: str, hf_token: str, username: str,
    ...
    reset_stop_event()  # Reset cancellation flag at the start
    ...
    for model_id in model_list:
        if stop_event.is_set():
            full_log += "[INFO] Quantization process stopped by user.\n"
            yield full_log
            return
...
# ---------------------------
# Stop Quantization Function
# ---------------------------
def stop_quantization():
    stop_event.set()
    return "[INFO] Stop signal sent. Quantization will halt shortly."
...
with gr.Blocks(title="SpongeQuant") as iface:
    ...
    with gr.Row():
        ...
        stop_button = gr.Button("Stop Quantization")
